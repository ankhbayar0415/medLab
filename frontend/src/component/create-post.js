  ...
  if (fileType.includes("image")) {
    return (
      <div className="file-viewer-container">
        ...
        />
        <div className="remove-file" style={style.removeFile} onClick={remove}>
          &times;
        </div>
      </div>
    );
...
function CreatePost(props) {
  ...
  const createPost = (e) => {
    ...
    axios
      .post(HOST_URL + "/create-post/", formData, options)
      .then((res) => {
        if (res.status === 200) {
          let data = res.data;
          if (data.status === 200) {
      ...
      <div className="create-post-main-container">
        <div className="create-post-container">
          <section className="create-post-section">
            <h2 style={{ color: "#c7c7c7", marginLeft: "10px" }}>
              Create Post
            </h2>
            <hr />

            <div className="create-post-form">
              <p style={{ color: "#c7c7c7", marginLeft: "20px" }}>
          ...
          <section className="selected-file-viewer">
            {Array.from(selectedFile).map((file, index) => {
              return (
                <FileViewer file={file} key={index} removeFile={removeFile} />
              );
            })}
          </section>
