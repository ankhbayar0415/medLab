...
function FollowAndFollowing(props) {
  ...
  let user = props.user;
  let account_visibility = props.user.account_visibility;
  ...
  React.useEffect(() => {
    ...
    axios
      .get(HOST_URL + "/is_following?userId=" + user.userId, {
        headers: {
  ...
  React.useEffect(() => {
    if (currentUid !== 0 && currentUid != undefined) {
      axios
        .get(HOST_URL + "/get_all_followers_ids", {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + Cookies.get("token"),
          },
      ...
      <div className={calledFrom}>
        ...
        >
          {!isAuth
            ...
            : isfollowing
            ? "Following"
            : Array.from(followers).includes(userId)
            ? "Follow back"
            : "Follow"}
        </button>
      </div>
...
export function RemoveFollower(props) {
  ...
  let [isClickedRemoveFollower, setIsClickedRemoveFollower] =
  ...
  return (
    <>
      {isClickedRemoveFollower ? (
        <div className="remove-follower-poup-alert-main-container">
          <div className="remove-follower-poup-alert-container">
            ...
            <div className="remove-follower-popup-alert-body">
              <h2 style={style.h2}>Remove Follower?</h2>
              <p style={style.p}>
                {"Instagram won't tell " +
                  user.username +
            ...
            <div className="remove-follower-popup-alert-footer">
              <div
                className="remove-follower-popup-alert-footer-confirm-btn"
                onClick={confirm}
              >
                Confirm
              </div>
              <div
                className="remove-follower-popup-alert-footer-cancel-btn"
                onClick={cancel}
              >
                Cancel
              </div>
      ...
      <div className="remove-following-poup-alert-main-container">
        <div className="remove-following-poup-alert-container">
          ...
          <div className="remove-following-popup-alert-footer">
            <div
              className="remove-following-popup-alert-footer-confirm-btn"
              onClick={confirm}
            >
              {isFollowing ? "Unfollow" : "Follow"}
            </div>
            <div
              className="remove-following-popup-alert-footer-cancel-btn"
              onClick={close}
            >
              Cancel
            </div>
      ...
      <div className="remove-post-poup-alert-main-container">
        <div className="remove-post-poup-alert-container">
          ...
          <div className="remove-post-popup-alert-body">
            <p style={style.p}>Do you want to delete this post?</p>
          </div>
          <div className="remove-post-popup-alert-footer">
            <div
              className="remove-post-popup-alert-footer-confirm-btn"
              onClick={confirm}
            >
              Delete
            </div>
            <div
              className="remove-post-popup-alert-footer-cancel-btn"
              onClick={close}
            >
              Cancel
            </div>
...
export function ConfirmationToReportPost(props) {
  let user = props.user;
  let close = props.close;
  let [isReported, setIsReported] = React.useState(true);
  let [isFailure, setIsFailure] = useState(false);
  let [isConfirmToReport, setIsConfirmToReport] = React.useState(false);

  useEffect(() => {
    if (isConfirmToReport) {
      if (Cookies.get("token") === undefined) {
        alert("You must be logged in to report this user");
        return;
      }
      axios
        .post(
          HOST_URL + "/report/post/",
          { notif_to: user.userId },
          {
            headers: {
              "content-type": "application/json",
              Authorization: "Bearer " + Cookies.get("token"),
    ...
    <>
      <div className="remove-post-poup-alert-main-container">
        <div className="remove-post-poup-alert-container">
          ...
          <div className="remove-post-popup-alert-body">
            <p style={style.p}>Do you want to report this post?</p>
          </div>
          <div className="remove-post-popup-alert-footer">
            <div
              className="remove-post-popup-alert-footer-confirm-btn"
...
export function RemoveFollowing(props) {
  ...
  let [isRemoved, setIsRemoved] = React.useState(false);
  let [isConfirmToRemove, setIsConfirmToRemove] = React.useState(false);
  let [isClickedRemoveFollowing, setIsClickedRemoveFollowing] =
    React.useState(false);
  let [isFollowing, setIsFollowing] = React.useState(true);
