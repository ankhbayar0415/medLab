...
function Home(props) {
  ...
  const filterPost = (newPosts) => {
    let filter = newPosts.filter(
      (ele, ind) =>
        ind === newPosts.findIndex((elem) => elem[0].postId === ele[0].postId)
    );
    return filter;
  };
  ...
  useEffect(() => {
    ...
    if (!reachMax)
      axios
        .get(HOST_URL + "/posts?offset=" + offset + "&limit=" + limit, {
          headers: {
  ...
  return (
    <>
      {isDeletedPost ? (
        <Toast message={"Post Deleted"} duration={duration} />
      ) : null}
        ...
        <section className="post-stories-section">
          <section className="post-sections">
            {isLoading ? (
              ...
              <Suggestion key={"suggestions"} type={"scroll-bar"} />
            ) : null}
            {isLoading ? (
              <>Loading...</>
            ) : (
            ...
            {isFooterLoading ? (
              <div className="post-load-more" onClick={loadMore}>
                <LoadMore className="load-more-button" /> &nbsp; Load More
              </div>
            ) : null}
          </section>
        </section>
