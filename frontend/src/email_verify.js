...
function EmailVerification(props) {
  ...
  let [message, setMessage] = useState(""); // eslint-disable-next-line
  ...
  useEffect(() => {
    for (let i = 0; i < codeLen; i++) {
      setInputFields((inputFields) => [
        ...inputFields,
        <InputField key={i} index={i} onchange={onchange} />,
      ]);
    }
    setLoading(false);
    return () => {};
  ...
  const resend = () => {
    axios
      .post(
        HOST_URL + "/accounts/email/resend",
        {
  ...
  const verify = () => {
    let vcode = code.join("");
    axios
      .post(HOST_URL + "/accounts/email/verify", {
        token: Cookies.get("email_verify_token"),
        code: vcode,
      })
      .then((res) => {
    ...
    <>
      <div className="verify-code-main-container">
        <div className="verify-container">
          <div className="verify-code-title">Verification Code</div>
          <div className="verify-input-container"> {inputFields}</div>
          <div className="verify-container-button">
            <button
              className="verify-code-button"
              onClick={verify}
              disabled={len === 6 ? false : true}
            >
              Verify
            </button>
            <button
