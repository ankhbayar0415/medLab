...
exports.getFollowerCount = function (userId) {
  let val = 0;
  const query =
    "SELECT COUNT(*) as count FROM followers WHERE followed_to=? AND isFollowRequest=?";
  return new Promise(function (resolve, reject) {
    connection.query(query, [parseInt(userId), val], function (err, result) {
      if (err) reject(err);
      try {
        if (result.length > 0) resolve(result[0].count);
      } catch (error) {
        reject(error);
      }
    });
...
exports.getFollowingCount = function (userId) {
  let val = 0;
  const query =
    "SELECT COUNT(*) as count FROM followers WHERE followed_by=? AND isFollowRequest=?";
  return new Promise(function (resolve, reject) {
    connection.query(query, [parseInt(userId), val], function (err, result) {
      if (err) reject(err);
      try {
...
exports.changePassword = function (
  ...
  if (newPassword !== confirmPassword) {
    ...
    return new Promise(function (resolve, reject) {
      connection.query(
        query,
        [parseInt(userId), md5(oldPassword)],
        function (err, result) {
          if (err) reject(err);
          resolve(result);
        }
      ...
      .then(function (result) {
        ...
        if (result.length > 0) {
          ...
          return new Promise(function (resolve, reject) {
            connection.query(
              query,
              [md5(newPassword), parseInt(userId)],
              function (err, result) {
                if (err)
                  resolve({
                    status: 400,
...
exports.getFollowers = function (userId) {
  const query =
    "SELECT user.username,user.fullname,user.userId,user.profile FROM user join followers ON user.userId=followers.followed_by WHERE followed_to=? AND isFollowRequest=?";
  return new Promise(function (resolve, reject) {
...
exports.getFollowing = function (userId) {
  const query =
    "SELECT user.username,user.fullname,user.userId,user.profile FROM user join followers ON user.userId=followers.followed_to WHERE followed_by=? AND isFollowRequest=?";
  return new Promise(function (resolve, reject) {
    connection.query(query, [parseInt(userId), 0], function (err, result) {
      if (err) reject(false);
      resolve(result);
...
exports.postStatus = function (userId, username, tags, files, caption) {
  ...
  const query =
    "INSERT INTO post(userId,postUrl,timestamp,caption) VALUES(?,?,?,?)";
  return new Promise(function (resolve, reject) {
    connection.beginTransaction(function (err) {
      ...
      connection.query(
        query,
        [parseInt(userId), postUrl, timestamp, caption],
        function (err, result) {
          if (err) {
            //console.logerr);
          ...
          let query1 =
            "INSERT INTO insta_media(postId,mimetype,url) VALUES(?,?,?)";
          for (let i = 0; i < len; i++) {
            connection.query(
              query1,
              [postId, files[i].mimetype, files[i].path],
              function (err, result) {
                if (err) {
                  //console.logerr);
                  return connection.rollback(function () {
...
getAllFollowingIds = function (userId) {
  const query =
    "SELECT user.userId FROM user join followers ON user.userId=followers.followed_to WHERE followed_by=? AND isFollowRequest=?";
  return new Promise(function (resolve, reject) {
    connection.query(query, [parseInt(userId), 0], function (err, result) {
      if (err) reject(err);
      resolve(result);
...
exports.getPosts = async function (userId, offset, limit) {
  ...
  return new Promise(function (resolve, reject) {
    connection.query(
      query,
      [followingIds, parseInt(offset), parseInt(limit)],
      function (err, result) {
        if (err) reject(err);
        resolve(result);
      }
      ...
      mysqlConnection.query(query1, [postId, currentUserId], (err, result) => {
        ...
        if (result.length == 0) {
          ...
          let query2 =
            "INSERT INTO " +
            table +
            "(postId,liked_by,pc_by,timestamp) VALUES(?,?,?,?)";
          mysqlConnection.query(
            query2,
            [postId, currentUserId, userId, timestamp],
            (err, result) => {
              if (err) reject(err);
              resolve(result);
            }
          ...
          //delete the like
          let query2 =
            "DELETE FROM " + table + " WHERE postId=? AND liked_by=?";
          mysqlConnection.query(
            query2,
...
exports.checkLike = function (postId, userId) {
  let table = "likes";
  let query =
    "SELECT * FROM " + table + " WHERE postId=? AND liked_by=? limit ?";
  return new Promise(function (resolve, reject) {
...
exports.commentPost = function (
  ...
  let query =
    "INSERT INTO " +
    table +
    "(postId,userId,comment_content,timestamp,parentId,comment_type,isModified) VALUES(?,?,?,?,?,?,?)";

  return new Promise(function (resolve, reject) {
    mysqlConnection.query(
      query,
      [postId, userId, comment, timestamp, parentCommentId, commentType, 0],
      (err, result) => {
        if (err) reject(err);
        resolve(result);
      }
...
exports.getMorePost = async function (postId, userId) {
  ...
  if (userInfo[0].account_visibility.toLowerCase() === "private") {
    return false;
  }
  return new Promise(function (resolve, reject) {
    let query = `SELECT post.postId,post.postUrl,post.userId,post.timestamp,post.caption,
...
const getAllFollowerIds = async function (userId) {
  const query =
    "SELECT user.userId FROM user join followers ON user.userId=followers.followed_by WHERE followed_to=? AND isFollowRequest=?";
  return new Promise(function (resolve, reject) {
    connection.query(query, [parseInt(userId), 0], function (err, result) {
      if (err) reject(err);
      resolve(result);
...
const getAllUserWhoFollowedByYourFollowingIds = async function (ids, userId) {
  let query =
    "SELECT DISTINCT  u.userId,u.username,u.profile,u.fullname,ui.account_visibility FROM user u  INNER JOIN userinfo ui ON ui.userId=u.userId INNER JOIN followers  f ON u.userId=f.followed_by WHERE u.userId NOT IN (?) AND u.userId!=?";
  return new Promise(function (resolve, reject) {
...
exports.getAllUserWhoFollowedByYourFollowingIds =
  getAllUserWhoFollowedByYourFollowingIds;

const getLimitedNewUsers = async function (currentUserId, userIds, limit) {
  const query =
    "SELECT user.userId,user.username,user.fullname,user.profile,ui.account_visibility FROM user INNER JOIN userinfo ui ON ui.userId=user.userId WHERE user.userId NOT IN (?) AND user.userId!=? ORDER BY user.created_date DESC LIMIT ?";
  return new Promise(function (resolve, reject) {
    connection.query(
      query,
      [userIds, currentUserId, limit],
      function (err, result) {
        if (err) reject(err);
        resolve(result);
      }
...
exports.isAdmin = function (userId) {
  let query = `SELECT user_type FROM user WHERE userId=? AND user_type=? LIMIT 1`;

  return new Promise((resolve, reject) => {
    mysqlConnection.query(query, [userId, "admin"], (err, result) => {
      if (err) reject(false);
      resolve(result);
    });
  })
...
exports.getNotifications = function (userId) {
  let query = `SELECT notifId, notif_by, notif_to, timestamp, notif_type FROM notifications WHERE notif_type=? AND notif_to=? ORDER BY timestamp DESC`;
  return new Promise((resolve, reject) => {
    mysqlConnection.query(query, ["like", userId], (err, result) => {
      if (err) reject(false);
      resolve(result);
    });
  })
...
exports.deleteNotif = function (userId) {
  let query = `DELETE FROM notifications WHERE notif_to=?`;
  return new Promise((resolve, reject) => {
    mysqlConnection.query(query, [userId], (err, result) => {
      if (err) reject(false);
      resolve(result);
...
exports.getReports = function (userId) {
  let query = `SELECT notifId, notif_by, notif_to, timestamp, notif_type FROM notifications WHERE notif_type=? ORDER BY timestamp DESC`;
  return new Promise((resolve, reject) => {
    mysqlConnection.query(query, ["report"], (err, result) => {
      if (err) reject(false);
      resolve(result);
    });
  })
...
exports.addLike = function (notif_by, notif_to) {
  let timestamp = new Date().getTime();
  let notif_type = "like";
  let query = `INSERT INTO notifications(notif_by,notif_to,timestamp,notif_type) VALUES(?,?,?,?)`;
  return new Promise((resolve, reject) => {
    mysqlConnection.query(
      query,
      [notif_by, notif_to, timestamp, notif_type],
      (err, result) => {
        if (err) reject(false);
        resolve(result);
      }
...
exports.reportUser = function (notif_by, notif_to) {
  let timestamp = new Date().getTime();
  let notif_type = "report";
  let query = `INSERT INTO notifications(notif_by,notif_to,timestamp,notif_type) VALUES(?,?,?,?)`;
  return new Promise((resolve, reject) => {
    mysqlConnection.query(
      query,
...
exports.generateCode = function (len) {
  var text = "";
  var possible =
          ...
          if (result1.status === 403) {
            ...
            mysqlConnection.query(query, [code, ucode], async (err, result) => {
              ...
              if (result.length > 0) {
                ...
                let newCode = this.generateCode(11);
    ...
    mysqlConnection.beginTransaction((err) => {
      if (err) {
        ...
        if (data.length > 0) {
          ...
          mysqlConnection.query(
            query,
            [d.postId, d.postUrl, d.userId, timestamp, d.caption],
            (err, result) => {
              if (err) {
                console.log(err);
                mysqlConnection.rollback(function () {
                ...
                for (let index = 0; index < data.length; index++) {
                  ...
                  mysqlConnection.query(
                    query,
                    [postId, data[index].mimetype, data[index].url],
                    (err, result) => {
                      if (err) {
                        console.log(err);
                        mysqlConnection.rollback(function () {
