...
exports.register = function (email, fullname, username, password) {
  return new Promise(function (resolve, reject) {
    ...
    var sql =
      "INSERT INTO user(email,username,fullname,user_type,created_date,verification,pswd,profile) VALUES(?,?,?,?,?,?,?,?)";
        ...
        mysqlConnection.query(
          ...
          function (err, result) {
            if (err) {
              ...
              let account_visibility = "Public";
              let account_type = "Personnel";
              let description = "";
              let lastmodified = new Date().getUTCMilliseconds();
              let account_status = "Active";
              let insertQ =
                "INSERT INTO userinfo(userId,account_visibility,account_type,description,date_of_birth,last_modified,account_status,website) VALUES(?,?,?,?,?,?,?,?)";
              mysqlConnection.query(
                insertQ,
                [
                  userId,
                ...
                function (err, res) {
                  if (err) {
                    ...
                    let code = helper.generateCode(6);
                    let value = md5(email + username + code, 64);

                    let insertQ =
                      "INSERT INTO emailverifier(email,cookiesvalue,code) VALUES(?,?,?)";
                    mysqlConnection.query(
                      insertQ,
                      [email, value, code],
                      (err, result) => {
...
exports.getUserInfo = function (userId) {
  const query =
    "SELECT user.fullname,user.email,user.user_type,user.verification,user.profile,userinfo.description,userinfo.date_of_birth,userinfo.account_status,userinfo.account_visibility,userinfo.gender,userinfo.website FROM user JOIN userinfo ON user.userId=userinfo.userId WHERE user.userId=?";
...
exports.getUserInfoByUsername = function (username) {
  const query =
    "SELECT user.userId,userinfo.website,user.username,user.fullname,user.user_type,user.profile,userinfo.description,userinfo.date_of_birth,userinfo.account_status,userinfo.account_visibility FROM user JOIN userinfo ON user.userId=userinfo.userId WHERE user.username=? AND user.userId=(SELECT userId FROM user WHERE username=?)";
...
exports.getUserInfoByUserId = function (userId) {
  const query =
    "SELECT userId,username,fullname,user.profile FROM user  WHERE userId=? LIMIT ?";
...
exports.followUser = function (userId, followingId) {
  ...
  return new Promise((resolve, reject) => {
    var checkQuery =
      "SELECT * FROM followers WHERE followed_by=? AND followed_to=? limit 1";
    ...
    mysqlConnection.query(
      ...
      function (err, result) {
        ...
        if (result.length > 0) {
          let deleteFollowing =
            "DELETE FROM followers WHERE followed_by=? AND followed_to=?";
          mysqlConnection.query(
            deleteFollowing,
            [userId, followingId],
            function (err, result) {
          ...
          let getUserInfo =
            "SELECT account_visibility FROM userinfo WHERE userId=?";
          mysqlConnection.query(
            getUserInfo,
            [followingId],
            function (err, result) {
              ...
              if (
                result[0].account_visibility.toLocaleLowerCase() !==
              ...
              var insertQuery =
                "INSERT INTO followers(followed_by,followed_to,timestamp,isFollowRequest) VALUES(?,?,?,?)";
              mysqlConnection.query(
                insertQuery,
                [userId, followingId, timestamp, isFollowedRequest],
                function (err, result) {
                  if (err) {
...
// check if current user is following user with userId
...
// update user info - 2 tables
exports.updateUserInfo = function (userId, userInfo) {
  return new Promise((resolve, reject) => {
    const query = "UPDATE user SET fullname=? WHERE userId=?";
    let timestamp = new Date().getTime();
    const query1 =
      "UPDATE userinfo SET description=?,last_modified=?,date_of_birth=?,website=?,gender=? where userId=?";
        ...
        function (err, result) {
          if (err) {
            mysqlConnection.rollback(function (err) {
              reject({
                error: "error in updating user information",
                status: 500,
              });
            });
          }
...
// check if the given email has account assocciated with it
        ...
        if (result.length > 0) {
          ...
          mysqlConnection.query(
            ...
            function (err, result) {
              if (err) reject(false);
              const query1 =
                "UPDATE passwordchanger SET old_password=? WHERE passwordChangeCode=? AND email=?";
              mysqlConnection.query(
                query1,
                [oldPassword, pcode, email],
                function (err, result) {
                  if (err) reject(false);
                  mysqlConnection.commit(function (err) {
                    if (err) {
