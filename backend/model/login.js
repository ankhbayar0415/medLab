...
exports.login = function (email, password) {
  return new Promise(function (resolve, reject) {
    mysqlConnection.query(
      "SELECT * FROM user where email=? AND pswd=? limit 1",
      [email, md5(password)],
      (error, result) => {
        if (error) {
          reject(error);
...
exports.usernameLogin = function (username, password) {
  return new Promise(function (resolve, reject) {
    mysqlConnection.query(
      "SELECT * FROM user where username=? AND pswd=? limit 1",
...
exports.relogin = function (email, password, userId, username) {
  return new Promise(function (resolve, reject) {
    mysqlConnection.query(
      "SELECT * FROM user where email=? AND pswd=? AND username=? AND userId=? limit 1",
